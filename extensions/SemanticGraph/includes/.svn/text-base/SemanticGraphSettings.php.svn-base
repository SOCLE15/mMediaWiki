	...
	function __construct() {
		...
		$this->options = array (
			'smm' => array (
				...
				'direction' => array ('type' => 'string' , 'default' => 'both', 'values' => array('incoming','outgoing','both'), 
					'help' => "The direction to follow for semantic properties"),
				...
				foreach ((array) $optionv['action'] as $action) { 
					switch ($action) {
						case 'toarray':
							...
							$argv[$optionk] = $z;
							break; // split on whitespace and comma combinations
						case 'nsproperty':
						case 'nscategory':
						case 'nsform':
						case 'nsimage':
